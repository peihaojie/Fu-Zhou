<template>
  <div id="q_statistics">
    ...
    <div class="top-box">
      <div>
        <div class="left-box">
          <p>{{ topBox.qualifiedCount?topBox.qualifiedCount:0 }}</p>
          <span>已完成</span>
        </div>
      ...
      <div>
        <div class="left-box">
          <p>{{ topBox.rectifyCount?topBox.rectifyCount:0 }}</p>
          <span>未整改</span>
        </div>
        <div class="right-box plaint1"></div>
      </div>
      <div>
        <div class="left-box">
          <p>{{ topBox.reviewCount?topBox.reviewCount:0 }}</p>
          <span>待复查</span>
        </div>
      ...
      <div>
        <div class="left-box">
          <p>{{ topBox.pastCount?topBox.pastCount:0 }}</p>
          <span>超期未整改</span>
        </div>
        <div class="right-box plaint2"></div>
      </div>
    ...
    <div class="main-box">
      <div class="time">
        ...
        <div class="time-box">
          ...
          ></el-date-picker>
          <!-- 按月选择 -->
          <el-date-picker style="display:none;" v-model="value4" type="month" placeholder="选择月"></el-date-picker>
          <!-- 按年选择 -->
          <el-date-picker style="display:none;" v-model="value5" type="year" placeholder="选择年"></el-date-picker>
          <!-- 自定义选择 -->
          <el-date-picker
            style="display:none;"
            v-model="value6"
          ...
          <div class="generalize-right">
            <ul>
              <li>
                检查总数
                <br />
                <span>{{ topBox.checkdSum?topBox.checkdSum:0 }}</span>
                项
              </li>
              <li>
                已完成
                <br />
                <span>{{ topBox.qualifiedCount?topBox.qualifiedCount:0 }}</span>
                项
              </li>
              <li>
                未完成
                <br />
                <span>{{ topBox.rectifyCount?topBox.rectifyCount:0 }}</span>
                项
              </li>
            </ul>
          </div>
      ...
      <!-- <div class="tendency">
        <div class="tendency-title">隐患趋势</div>
...
#q_statistics {
  .top-box {
    ...
    > div {
      ...
      height: 1.67rem;
      justify-content: space-between;
      padding: 0.48rem 0.32rem 0.47rem 0.42rem;
  ...
  .main-box {
    ...
    height: 7.34rem;
    margin-top: 0.4rem;
    padding: 0.34rem 0.32rem 0 0.22rem;
...
export default {
  ...
  methods: {
    ...
    // getSelectPolling() {
    //   this.$axios
    //     .post(`/lz/polling/selectPolling?projectId=${this.pid}`)
    //     .then(res => {
    //       let date = [];
    //       let accomplish = [];
    //       let unfinished = [];
    ...
    //         if (
    //           date.indexOf(res.data.msg[i].deadlineTime.split(" ")[0]) == -1
    //         ) {
    //           date.push(res.data.msg[i].deadlineTime.split(" ")[0]);
    //         }
    ...
    getStatisticsCount() {
      this.$axios
        .post(`/api/safetyPcApi/statisticsCount?projectId=${this.pid}&differentiate=1`)
        .then(res => {
          this.topBox = res.data.data
        })
    }
  }
};
