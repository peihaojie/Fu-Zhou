      ...
      <div class="list-box">
        ...
        <ul class="list-body">
          <li
            ...
            :key="index"
            @click="$router.push({ path: '/q_particulars', query: { safetyId: item.safetyId } })"
          >
            <div class="issue">
              <p>{{item.safetyDescribe}}</p>
              <span :class="item.gradeName=='轻微'?'slight':item.gradeName=='一般'?'ordinary':'severity'"></span>
...
export default {
  ...
  methods: {
    ...
    getInspectionRecordList() {
      ...
      this.$axios
        .post(
          `/api/safetyPcApi/getInspectionRecordList?projectId=${this.pid}&differentiate=1&pageNum=${this.pageNum}&pageSize=${this.pageSize}&hiddenId=${this.hiddenId}&status=${this.status}&initiatorTime=${this.initiatorTime}&initiatorName=${this.initiatorName}&rectifyName=${this.rectifyName}&reviewName=${this.reviewName}&constructionId=${this.constructionId}&areaId=${this.areaId}&problemGradeId=${this.problemGradeId}`
        )
        .then(res => {
