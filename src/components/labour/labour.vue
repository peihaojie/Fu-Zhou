          ...
          <div class="left-top-data">
            <ul>
              ...
              <li>
                ...
                <div>
                  <p>今日出勤人数</p>
                  <span>{{Number(KQCount.workerCheck)+Number(KQCount.managerCheck)}}</span>
                </div>
              </li>
              <!-- <li>
                <div>
    ...
    <div class="bottom">
      ...
      <div class="procedure">
            ...
            <div>
              ...
              <li>
                ...
                <span
                  ...
                  :class="contract.workConfirm.hg=='合格'?'font-green':'font-red'"
                >{{contract.workConfirm.hg}}</span>
              </li>
            </div>
          </ul>
        </div>
        ...
        >
          ...
          <div
            class="border"
            :class="contract.workConfirm.hg=='合格'?'border-green':'border-red'"
            v-if="contract.workConfirm"
          >
            ...
            <span
              v-if="contract.workConfirm.bfb.length>0"
              id="roateBfb1"
            >{{contract.workConfirm.bfb}}</span>
            <span v-else>0</span>
          </div>
        </div>
      </div>
      ...
      <div class="contract">
        ...
        <div class="float-left">
          <ul style="margin-top:-.1rem">
            <li>
              <span class="span-margin">已退场:</span>
              <span v-if="contract.entrance">{{contract.entrance.total}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">已签订:</span>
            ...
            <li>
              <span class="span-margin">是否合格:</span>
              <span
                v-if="contract.entrance"
                :class="contract.entrance.hg=='合格'?'font-green':'font-red'"
              >{{contract.entrance.hg}}</span>
            </li>
          </ul>
        </div>
        <div
        ...
        >
          <div
            class="border"
            :class="contract.entrance.hg=='合格'?'border-green':'border-red'"
            v-if="contract.entrance"
          >
      ...
      <div class="safety">
            ...
            <div>
              ...
              <li>
                ...
                <span
                  ...
                  :class="contract.contract.hg=='合格'?'font-green':'font-red'"
                >{{contract.contract.hg}}</span>
              </li>
            </div>
          </ul>
        </div>
        ...
        >
          <div
            class="border"
            :class="contract.contract.hg=='合格'?'border-green':'border-red'"
            v-if="contract.contract"
          >
...
export default {
  ...
  methods: {
    ...
    // 获取合同签订数据
    getDataCount() {
      this.$axios.get(`/lz/get/getDataCount?pid=${this.pid}`).then(res => {
    ...
    // 获取现场人员出勤数据
    getKQCountData() {
      this.$axios.get(`/lz/get/getKQCount?pid=${this.pid}`).then(res => {
    ...
    // 获取现场工种人数
    getWorkTypeData() {
      this.$axios.get(`/lz/get/getWorkType?pid=${this.pid}`).then(res => {
    ...
    // 获取班组与人员考勤数据
    getTeamCountData() {
      this.$axios.get(`/lz/get/getTeamCount?pid=${this.pid}`).then(res => {
        // console.log(res.data)
    ...
    getNearlyEightDaysData() {
      this.$axios
        .get(`/lz/get/getNearlyEightDays?pid=${this.pid}`)
        .then(res => {
          // console.log(res.data)
          let aMTotal = [];
          let aMZc = [];
          ...
          for (let i2 = 0; i2 < aMZc.length; i2++) {
            for (let i3 = 0; i3 < aMZc.length; i3++) {
              if (
                aMZc[i2] > this.attendanceMax ||
                aMZcGly[i2] > this.attendanceMax ||
                aMTotal[i2] > this.attendanceMax
              ) {
    ...
    // 获取今日劳动曲线数据
    getXSData() {
      this.$axios.get(`/lz/get/getXS?pid=${this.pid}`).then(res => {
        console.log(res.data);
        let lMZc = [];
        let lMDay = [];
    ...
    getBuildcompanyData() {
      this.$axios
        .get(`/lz/get/getBuildcompanyData?pid=${this.pid}`)
        .then(res => {
    ...
    getKQCount() {
      this.$axios
        .post(
          `/api/pc/projectWorkersApi/getKQCount?projectId=${this.projectId}`
        )
        .then(res => {
          // console.log(res.data)
    ...
    getWorkType() {
      this.$axios
        .post(
          `/api/attendanceRecordApi/getWorkType?projectId=${this.projectId}`
        )
        .then(res => {
          // console.log(res.data)
    ...
    getTeamCount() {
      this.$axios
        .post(
          `/api/attendanceRecordApi/getTeamCount?projectId=${this.projectId}`
        )
        .then(res => {
          ...
          if (res.data.team.length) {
            ...
            for (let i = 0; i < res.data.team.length; i++) {
              temp.push({
                value: res.data.team[i].kq,
                name: res.data.team[i].title
              });
            }
          }
    ...
    getNearlyEightDays() {
      this.$axios
        .post(
          `/api/attendanceRecordApi/getNearlyEightDays?projectId=${this.projectId}`
        )
        .then(res => {
          // console.log(res.data)
          let date = [];
    ...
    getXS() {
      this.$axios
        .post(`/api/attendanceRecordApi/getXS?projectId=${this.projectId}`)
        .then(res => {
    ...
    getBuildCompany() {
      this.$axios
        .post(
          `/api/attendanceRecordApi/getBuildcompanyData?projectId=${this.projectId}`
        )
        .then(res => {
          // console.log(res.data)
          this.buildCompany = res.data.buildcompany;
          if (this.buildCompany.length > 2) {
            this.scrollStart("rightBottom", "rightBottom1", "rightBottom2");
          }
        });
    },
    ...
    getContract() {
      this.$axios
        .post(
          `/api/pc/projectWorkersApi/getDataCount?projectId=${this.projectId}`
        )
        .then(res => {
          this.contract = res.data;
          if (this.contract.entrance.total == this.contract.entrance.ht) {
            this.contract.entrance.hg = '合格'
            this.contract.entrance.bfb = '100%'
          } else {
            this.contract.entrance.hg = '不合格'
            if (this.contract.entrance.ht == 0) {
              this.contract.entrance.bfb = '0%'
            } else {
              this.contract.entrance.bfb = Math.round(Number(this.contract.entrance.ht) / Number(this.contract.entrance.total) * 100) + '%'
            }
          }
        });
    }
...
#labour .bottom .information {
  height: 100%;
  width: 9.1rem;
  background-image: url("../../../static/images/twoSystem_bottom_background.png");
  background-size: contain;
}
#labour .bottom .contract,
#labour .bottom .safety {
...
#labour .bottom .procedure .lt {
    ...
    top: -3px;
    left: -3px;
    border-top: 2px solid #3375fe;
    border-left: 2px solid #3375fe;
  }

  #labour .bottom .procedure .rt {
    position: absolute;
    display: block;
    width: 15px;
    height: 15px;
    ...
    right: -3px;
    border-top: 2px solid #3375fe;
    border-right: 2px solid #3375fe;
  }

  #labour .bottom .procedure .lb {
    position: absolute;
    display: block;
    width: 15px;
    height: 15px;
    bottom: -3px;
    left: -3px;
    border-bottom: 2px solid #3375fe;
    border-left: 2px solid #3375fe;
  }

  #labour .bottom .procedure .rb {
    position: absolute;
    display: block;
    width: 15px;
    height: 15px;
