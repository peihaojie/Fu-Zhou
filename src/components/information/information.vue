...
export default {
    ...
    methods: {
        // 获取文件列表
        getQueryFolderData() {
            this.$old.post(`/lz/folder/queryFolder?projectId=${this.pid}`).then(
                res => {
        ...
        saveClick() {
            ...
            // 设置文件夹名字
            this.$old.post(`/lz/folder/addFolder?folderName=${this.folderName}&projectId=${this.pid}&parentId=${this.folderId}`).then(
                res => {
                    // console.log(res.data)
        ...
        uploadObject() {
            ...
            this.$old.post(`/lz/file/uploadObject?folderName=1&folderId=${this.folderId}&uploader=${this.pid}`,this.file,headers).then(
                res => {
        ...
        // 遍历文件列表
        getFileDataList() {
            this.$old.post(`/lz/file/queryFile?page=${this.page}&pageSize=${this.pageSize}&uploader=${this.pid}`).then(
                res => {
        ...
        // 删除文件夹点击事件
        getDeleteFolder() {
            this.$old.post(`/lz/folder/deleteFolder?folderId=${this.folderId}`).then(
                res => {
                    console.log(res.data)
                    // 删除成功后更新文件夹列表
                    this.getQueryFolderData()
        ...
        deletFile(fileId) {
            console.log(fileId)
            this.$old.post(`/lz/file/deleteFile?ids=${fileId}`).then(
                res => {
        ...
        fuzzySearch() {
            if(this.fileName!='') {
                this.$old.post(`/lz/file/queryFile?page=${this.page}&pageSize=${this.pageSize}&uploader=${this.pid}&fileName=${this.fileName}`).then(
                    res => {
                        // console.log(res.data.rows)
                        this.fileDataList = res.data.rows
                        let tempTableData = []
