    ...
    <div id="right">
      ...
      <div class="up">
        ...
        <div class="two">
          ...
          <div id="sixworter" style="width: 3.6rem;height:3rem;" class="sixworter"></div>
        </div>
      </div>
      <div class="down">
        <h3>本月用水管理</h3>
        ...
        <div class="one">
          ...
          <div id="sevenworter" style="width: 3.6rem;height:3rem;" class="sevenworter"></div>
        </div>
        <!-- <div class="two">
          <div class="mintitle">
            <span>最近6月用水</span>
...
export default {
  ...
  methods: {
        ...
        series: [
          {
            ...
            symbolSize: 4,
            color: ["#508bb8"],
            data: [24.6, 23.1, 20.9, 21.2, 25.3, 21.1, 21.5]
            // data: sevenshuiV
          }
        ]
      });
        ...
        series: [
          {
            ...
            symbolSize: 4,
            color: ["#508bb8"],
            data: [571.6, 593.4, 563.6, 621.3, 619.6, 594.9, 513.8]
            // data: sixshuiV
          }
        ]
      });
    ...
    // 获取用水时间
    getWaterTime() {
      var month = Number(this.date.split('月')[0])
      // var day = Number(this.date.split(' ')[1].split('日')[0])
      ...
      for (let i = 0; i < 7; i++) {
        ...
        monthList.unshift(month + '月')
        month--
      }
      console.log(monthList)
      for (let i = 0; i < 7; i++) {
        var temp = new Date(day)
        dayList.unshift(temp.getDate() + '日')
        day -= 86400000
      }
      console.log(dayList)
      this.sevenworter(dayList)
      this.sixworter(monthList)
    },

    // 获取温度
    getTemperature() {
      this.$axios
        .post('https://www.tianqiapi.com/api/?version=v1&appsecret=PDsIqZt7&appid=74243526&city=永泰')
        .then(res => {
          var temp = res.data.data[0].hours
          this.temperatureList['2'] = Number(temp[0].tem.split('℃')[0]) - Math.floor(6*Math.random())
          this.temperatureList['5'] = Number(temp[0].tem.split('℃')[0]) - Math.floor(3*Math.random())
          let j = 5;
          for (let i = 0; i < temp.length-2; i++) {
            j = j + 3;
            this.temperatureList[j.toString()] = Number(temp[i].tem.split('℃')[0]);
          }
          this.getTimes()
        })
    },
    ...
    makeData(hourTime) {
      ...
      if(this.temperatureList[hourTime - 1]) {
        return this.temperatureList[hourTime - 1] - Math.floor(3*Math.random());
      }
      return this.makeData(hourTime - 1);
    },
    ...
    getTimes() {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        temp.unshift(hourTime);
        if(!this.temperatureList[hourTime]) {
          this.temperatureList[hourTime] = this.makeData(hourTime)
        }
        data.unshift(this.temperatureList[hourTime]);
        hourTime--
      }
      // console.log(temp)
      // console.log(data)
      this.temperaturePic(temp, data)
      // setInterval(() => {
      //   this.time = moment().format("HH:mm");
      // }, 60000);
    },
